...
export async function addManualTeamsMeetingAction(input: { code: string; subject: string; startsAt: string; durationMinutes: number; staffIds: string[]; joinUrl: string }) {
  ...
  const html = wrapEmail(subject, `<p>تمت دعوتك لاجتماع Microsoft Teams مرتبط بالطلب <b dir="ltr">${escapeHtml(ticket.code)}</b>.</p><p><b>العميل:</b> ${escapeHtml(ticket.client_name)}</p><p><b>الموعد:</b> ${escapeHtml(start.toLocaleString("ar-EG"))}</p><p><a href="${escapeHtml(parsedUrl.toString())}" style="display:inline-block;background:#4f46e5;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none;font-weight:700">الانضمام إلى اجتماع Teams ↗</a></p>`, settings);
