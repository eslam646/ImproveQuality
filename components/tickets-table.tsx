...
export function TicketsTable({ rows, readOnlyNote, storageKey = "support-hub-ticket-columns" }: { rows: Ticket[]; readOnlyNote?: string; storageKey?: string }) {
  ...
  const cell = (k: ColumnKey, t: Ticket) => {
    ...
    if (k === "kind") return <td className="px-4 py-2.5 text-xs"><div className={`mb-1 w-fit rounded-full px-2 py-0.5 font-bold ${t.ticket_kind === "instant_support" || t.is_urgent ? "bg-rose-100 text-rose-700" : "bg-slate-100 text-slate-600"}`}>{t.ticket_kind === "instant_support" || t.is_urgent ? "🚨 دعم فوري" : TICKET_KIND_LABELS.standard}</div><div className="text-slate-500">{REQUEST_TYPE_LABELS[t.request_type ?? "issue"]}</div></td>;
