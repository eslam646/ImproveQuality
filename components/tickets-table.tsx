...
import { ASSIGNMENT_STATUS_LABELS, REQUEST_TYPE_LABELS, STATUS_COLORS, STATUS_LABELS, TICKET_KIND_LABELS, urgentStatusInfo } from "@/lib/labels";
...
export function TicketsTable({ rows, readOnlyNote, storageKey = "support-hub-ticket-columns", specialistsByTicket = {} }: { rows: Ticket[]; readOnlyNote?: string; storageKey?: string; specialistsByTicket?: Record<string, string> }) {
  ...
  const cell = (k: ColumnKey, t: Ticket) => {
    ...
    // «آخر تحديث» = آخر أي حركة على التذكرة (يتطابق مع ترتيب «الأحدث تعديلاً») — وليس آخر تغيير حالة فقط
    return <td className="px-4 py-2.5 text-xs text-slate-500">{fmtDate(t.updated_at ?? t.last_status_change)}</td>;
  };
